...
description: >
  ...
  cobre múltiplas features independentes, detecta automaticamente e propõe divisão
  em PRDs separados, criando-os em sequência na mesma sessão. Cada PRD é autocontido
  ...
  ou documentar requisitos — mesmo que não use o termo "PRD" explicitamente. Também
  quando mencionar "requisitos do produto", "spec de feature", "escopo da feature",
  "documento de requisitos", "refinar PRD", "ajustar PRD", ou fornecer uma descrição longa
  de funcionalidades para especificar.
---

# Escrever PRD

Cria e edita PRDs de feature independentes e autocontidos **em comportamento e regra de
negócio**. Cada PRD descreve a feature de forma completa do ponto de vista de negócio; o
contexto técnico para implementar vem do TRD (carregado junto na implementação). O campo de
...
- **Separação de responsabilidades** — PRD cobre comportamento e regra de negócio; TRD e ADR cobrem restrições técnicas, stack e decisões de arquitetura
...
Analisar se o input cobre features independentes entre si. Sinais:
- Domínios distintos sem dependência (ex: "autenticação + perfil + notificações")
- Milestones que poderiam ser entregues em qualquer ordem
- Mais de 6 funcionalidades explícitas sem encadeamento claro

Se detectar múltiplas features independentes, **propor split concreto antes de gerar**:

```
Detectei 3 features independentes no input. Sugiro criar:
...
- Manter milestones entre 3 e 6 — cada um definido por **quais USs cobre** (por ID) e por um **critério de conclusão** (condição observável + aprovador), independentemente entregável quando possível. Não detalhar tarefas técnicas no milestone — a decomposição (passos, validação, ordem) é do `criar-plan`
